...
def import_csv():
    ...
    try:
        ...
        if records:
            cursor.executemany("""
                REPLACE INTO sales (order_id, timestamp, item_name, category, quantity, unit_price, total_price, payment_method, order_type, status)
        ...
        return jsonify({"error": f"Database Offline: {str(e)}. Please check your Aiven Cloud console."}), 500
    except pymysql.err.IntegrityError as e:
        return jsonify({"error": f"Database Error: {str(e)}. You might be importing duplicate Order IDs."}), 500
    except Exception as e:
